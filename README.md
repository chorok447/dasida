...
실제 서버 배포는 아직 미구현(CD workflow placeholder). 상세·**Docker Hub 검증·amd64 platform 주의**는 [`apps/api/docs/backend/container-images.md`](apps/api/docs/backend/container-images.md) 참고. **Nginx reverse proxy 배포안**은 [`nginx-reverse-proxy-deployment.md`](apps/api/docs/backend/nginx-reverse-proxy-deployment.md).
