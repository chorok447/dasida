...
관련: [github-secrets-and-environments.md](./github-secrets-and-environments.md), [main-release-readiness.md](./main-release-readiness.md), [deploy/.env.prod.example](../../../../deploy/.env.prod.example).
...
| name | GitHub target | required before main merge | required before deploy | validation rule | owner / decision needed |
|------|---------------|---------------------------|------------------------|-----------------|-------------------------|
| `GHCR_API_IMAGE` | Repository Variable | no | no | 기본 `ghcr.io/chorok447/dasida-api` 와 일치하면 생략 가능 | — |
| `GHCR_WEB_IMAGE` | Repository Variable | no | no | 기본 `ghcr.io/chorok447/dasida-web` 와 일치하면 생략 가능 | — |
