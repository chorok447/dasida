...
### Web API URL (브라우저 vs SSR)

| 변수 | 용도 | 예시 |
|------|------|------|
| `NEXT_PUBLIC_API_URL` | 브라우저 fetch (Web image **build arg**) | `https://api.example.com` |
| `API_INTERNAL_URL` | web 컨테이너 SSR (compose **런타임 env**) | `http://api:8080` |

web 컨테이너 안에서 `127.0.0.1:8080`은 web 프로세스 자신을 가리킨다. SSR 이 API 데이터를 가져오려면 compose 내부 DNS(`api`)를 써야 한다. 브라우저는 public URL(Nginx 경유)을 쓴다.
...
- restore 절차는 [mysql-backup-restore-runbook.md](./mysql-backup-restore-runbook.md) 참고.
...
| `API_INTERNAL_URL` | `http://api:8080` (web 컨테이너 SSR, 런타임 env) | 동일 |
...
- [ ] MySQL backup/restore runbook — [mysql-backup-restore-runbook.md](./mysql-backup-restore-runbook.md) (문서화 완료, 실제 drill 미실행)
- [ ] host Nginx + TLS runbook
- [ ] `NEXT_PUBLIC_API_URL` / `APP_CORS_ALLOWED_ORIGINS` 확정·주입
- [ ] production deploy runbook 작성 — [single-vm-production-deploy-runbook.md](./single-vm-production-deploy-runbook.md) (문서화 완료, 실제 deploy 미실행)
