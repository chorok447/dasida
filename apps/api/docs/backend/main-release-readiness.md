...
| Production Docker image (Docker Hub) | **구현됨** — main push 시 `<DOCKERHUB_USERNAME>/dasida-api` / `dasida-web` push |
...
| 항목 | 형태 | 설명 | prod 준비 |
|------|------|------|-----------|
| `DOCKERHUB_USERNAME` | Repository Variable | Docker Hub namespace | [ ] main push 전 등록 |
| `DOCKERHUB_TOKEN` | Repository Secret | Docker Hub access token (main push login) | [ ] main push 전 등록 |
...
| 항목 | 확인 | 체크 |
|------|------|------|
| Image 이름 | `docker.io/<DOCKERHUB_USERNAME>/dasida-api`, `docker.io/<DOCKERHUB_USERNAME>/dasida-web` | [ ] |
| Tags (main push) | `main`, `sha-<shortsha>` | [ ] |
| Repository visibility | public/private 정책 결정(운영 서버 pull 가능해야 함) | [ ] |
| Push credential | `DOCKERHUB_USERNAME` + `DOCKERHUB_TOKEN` (main push job) | [ ] |
...
| 방식 | 적합성 | 비고 | 결정 |
|------|--------|------|------|
| Docker Compose on server | 소규모·단일 서버 | Docker Hub pull + compose + env file; template: [`deploy/compose.prod.example.yml`](../../../../deploy/compose.prod.example.yml) | [ ] **1차 추천** |
...
| 단계 | 기대 | 체크 |
|------|------|------|
| Docker Hub push | `dasida-api` / `dasida-web` 에 `main`, `sha-*` tag 생성 | [ ] |
...
| 항목 | 방법 | 체크 |
|------|------|------|
| Docker Hub image 존재 | `docker.io/<DOCKERHUB_USERNAME>/dasida-api:sha-…` 등 | [ ] |
