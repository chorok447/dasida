...
분류·Environment 전략·생성 runbook: [github-secrets-and-environments.md](./github-secrets-and-environments.md). 운영 값 수집 체크리스트: [production-env-values-template.md](./production-env-values-template.md).
...
| Rate limit / denylist store | prod 는 **redis 고정** (`application-prod.yml`) | Redis 인프라 provisioning·접속 값 주입 ([정책](./redis-security-store-policy.md)) | [x] 설정 완료 |
...
**1차 추천안**: Docker Compose on VM + **host Nginx** + **single VM stack**(web/api/mysql/redis) — [deployment-strategy.md](./deployment-strategy.md), [nginx-reverse-proxy-deployment.md](./nginx-reverse-proxy-deployment.md), [single-vm-production-deploy-runbook.md](./single-vm-production-deploy-runbook.md).
...
- [ ] single VM 전략 확인: [single-vm-production-deploy-runbook.md](./single-vm-production-deploy-runbook.md) (mysql/redis volume·backup·최소 스펙)
...
| CD image push | main push CI **성공 후에만** Docker Hub push (`workflow_run` 게이트) | [x] |
...
- [ ] prod MySQL·(필요 시) Redis 인프라 미구축 (single VM compose 또는 managed — [single-vm-production-deploy-runbook.md](./single-vm-production-deploy-runbook.md))
