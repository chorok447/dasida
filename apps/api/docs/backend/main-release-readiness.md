...
| **MySQL 8** | DB 생성, 네트워크 접근, 백업·복구 절차 | [ ] — [single-vm-compose-deployment.md](./single-vm-compose-deployment.md) (compose volume·backup) |
...
| 방식 | 적합성 | 비고 | 결정 |
|------|--------|------|------|
| Docker Compose on server | 소규모·단일 서버 | Docker Hub pull + compose(web/api/mysql/redis) + host Nginx; template: [`compose.prod.example.yml`](../../../../deploy/compose.prod.example.yml) + 서버 override | [ ] **1차 추천** |
