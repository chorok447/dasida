...
import com.dasida.api.notification.NotificationSeed
...
/**
 * 비어있는 테이블에만 시드 적재. reversed() 로 저장해 seq DESC 정렬 시 시드 원래 순서(p1, c1, n1 …)가 유지된다.
 */
@Component
class SeedRunner(
    ...
    override fun run(vararg args: String?) {
        ...
        if (notifications.count() == 0L) {
            notifications.saveAll(NotificationSeed.notifications.reversed().onEachIndexed { i, n -> n.seq = (i + 1).toLong() })
        }
    }
}
